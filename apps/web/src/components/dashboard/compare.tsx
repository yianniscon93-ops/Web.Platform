...
export function SlotChip({
  ...
  return (
    <span
      className="inline-flex items-center gap-2 rounded-full pl-3 pr-2 py-1.5 text-[13px] glass-card"
    ...
    <div className="overflow-x-auto ps-scroll">
      <table className="w-full text-left" style={{ borderCollapse: "separate", borderSpacing: 0 }}>
        <thead>
          <tr>
            <th
              className="text-[12px] uppercase tracking-wider font-semibold py-2 pr-4"
            ...
            {slots.map((s) => (
              <th
                key={s.label}
                className="text-[14px] font-bold py-2 pr-4 whitespace-nowrap"
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.label}>
              <td
                className="py-2.5 pr-4 text-[14px] font-medium whitespace-nowrap"
              ...
              {slots.map((s, i) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`text-[15px] ${isBest ? "font-bold" : "font-medium"} inline-flex items-center gap-1.5 rounded-md ${isBest ? "px-1.5 py-0.5" : ""}`}
                      style={{
                        color: UI.text,
                    ...
                    >
                      {r.values[i] ?? "—"}
                      {isBest && (
                        <span className="text-[12px] font-bold uppercase tracking-wider" style={{ color: UI.green }}>
                          best
                        </span>
                      )}
                    </span>
                    {r.hints?.[i] && (
                      <span className="block text-[13px] mt-0.5" style={{ color: UI.faint }}>
                        {r.hints[i]}
                      </span>
                    )}
                  </td>
