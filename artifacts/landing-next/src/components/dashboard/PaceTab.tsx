...
function ComparePace({ slots }: { slots: SlotView[] }) {
  ...
  return (
    <div>
      <div
        className="flex items-center gap-2.5 rounded-xl px-4 py-3 mb-2.5 text-[13px] flex-wrap"
        style={{ background: "rgba(255,255,255,0.04)", border: `1px solid ${UI.border}`, color: UI.text }}
      >
        <Info size={15} style={{ color: UI.green }} className="shrink-0" />
        <span>Booking stats follow each area&apos;s scope and your filters:</span>
        {slots.map((v) => (
          <span key={v.id} className="inline-flex items-center gap-1.5 text-[12px]" style={{ color: UI.muted }}>
      ...
      <div className="glass-card rounded-2xl p-5 mt-2.5">
        ...
        <div className="flex flex-col gap-3">
          {slots.map((v, i) => {
            ...
            return (
              <div key={v.id}>
                <p className="text-[11px] font-semibold mb-1.5 flex items-center gap-1.5" style={{ color: UI.muted }}>
                  <SlotDot color={v.color} dash={v.dash} />
                  {v.label}
                </p>
                {mix ? (
        ...
        <div className="glass-card rounded-2xl p-5 mt-2.5">
          ...
          >
            {stayWeeks.map((week) => {
              ...
              return (
                <div key={week}>
                  <p className="text-[11px] font-semibold mb-2" style={{ color: UI.muted }}>
